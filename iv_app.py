...
# # --- PAGE CONFIG ---
# st.set_page_config(page_title="IV-Curve Explorer", layout="wide")

# # --- LOAD DATA ---
# PARQUET_PATH = Path("../iv-database-1.parquet")
# # Path logo sesuai permintaan
...
# # 2. Pilih Serial Number (Filtrasi berdasarkan Site)
...
# # 3. Pilih Tanggal (Filtrasi berdasarkan Site & Serial)
...
#         ax.plot(chunk["Voltage_V"], chunk["Current_A"], marker="o", 
#                 linewidth=1.5, markersize=3, label=f"PV {pv}", alpha=0.7)

#     ax.set_title(f"Combined IV Curves | {serial_selected} | {date_selected}", fontsize=12)
#     ax.set_xlabel("Voltage (V)")
#     ax.set_ylabel("Current (A)")
#     ax.grid(True, linestyle='--', alpha=0.4)
#     ax.legend(title="PV Input", bbox_to_anchor=(1.01, 1), loc='upper left', ncol=ceil(len(pvs)/15))
#     plt.tight_layout()
#     return fig

# def plot_iv_grid(df_file):
#     pvs = sorted(df_file["PV_Input"].unique(), key=_pv_sort_key)
#     n = len(pvs)
#     rows = ceil(sqrt(n))
#     cols = ceil(n / rows)
...
#     vmin, vmax = df_file["Voltage_V"].min(), df_file["Voltage_V"].max()
#     imin, imax = df_file["Current_A"].min(), df_file["Current_A"].max()

#     for i, pv in enumerate(pvs):
#         chunk = df_file[df_file["PV_Input"] == pv].sort_values(["Voltage_V", "data_point"])
#         axes[i].plot(chunk["Voltage_V"], chunk["Current_A"], marker="o", color="#2c3e50", markersize=2)
#         axes[i].set_title(f"PV: {pv}", fontsize=9, fontweight='bold')
#         axes[i].set_xlim(vmin * 0.9, vmax * 1.1)
#         axes[i].set_ylim(imin * 0.9, imax * 1.1)
...
#     with tab1:
#         st.pyplot(plot_iv_combined(dff))

#     with tab2:
#         st.pyplot(plot_iv_grid(dff))
# else:
